  ...
  if (randomPosition) {
    for (i=0; i<NUM_LEDS; i++) {
      ...
      if (distanceFromCenter >= (height/2)) {
        ...
        leds[i] = ColorFromPalette(currentPalette, 90 + map(distanceFromCenter, 0, NUM_LEDS-1, 0, 255), 100, LINEARBLEND);
      }
    }
    ...
    for (i=0; i<NUM_LEDS; i++) {
      ...
      else leds[i] = ColorFromPalette(currentPalette, 90 + map(i, 0, NUM_LEDS-1, 0, 255), 100, LINEARBLEND);
    }
    // Draw peak dot  
